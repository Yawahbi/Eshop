		...
		if($do=='Manage'){ // debut de if
			...
			if($count > 0){ ?>
					...
					<div class="signup-container">
						...
						<form class="pt-5 pb-4" action="?do=Update&userid=<?php echo $userid; ?>" method="post">
						    ...
						    <input class="form-control" type="hidden" name="old-password" autocomplete="new-password" value="<?php echo $row['password']; ?>">
						   	<div class="input-group mb-3">
							  	<span class="input-group-btn signup-bg-color">
						         	<span class="btn" style="font-size: 0.9rem;">Nom</span>
						        </span>
				...
				if(strlen($username) > 20){
					$formErrors[]="<div class='alert alert-danger'>"."Le nom d'utilisateur doit comporter moins de <strong> 20 caractères </strong>"."</div>";
				}
				if(empty($username)){
					$formErrors[]="<div class='alert alert-danger'>"."Le nom d'utilisateur ne doit pas être vide"."</div>";
				}
				if(strlen($username) < 3){
					$formErrors[]="<div class='alert alert-danger'>"."Le nom d'utilisateur doit contenir plus de <strong> 3 caractères </strong>"."</div>";
				}
				if(empty($email)){
					$formErrors[]="<div class='alert alert-danger'>"."L'e-mail ne doit pas être vide"."</div>";
				}
				if(empty($fullName)){
					$formErrors[]="<div class='alert alert-danger'>"."Full Name mustn't be empty"."</div>";
				}
				...
				if(empty($formErrors)){
					...
					$message="<div class='alert alert-success'> ".$stmt->rowCount()."Mise à jour terminée"."</div>";
				...
				$message="<div class='alert alert-danger'>"."Vous n'avez pas accès à cette page"."</div>";
